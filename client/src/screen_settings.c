...
static void build_account_menu(GtkWidget *main)
{
    ...
    gtk_widget_set_size_request(GTK_WIDGET(t_settings_scr.change_info), w-305, h);
    ...
    gtk_widget_set_size_request(GTK_WIDGET(t_settings_scr.select_avatar), w-305, h);

    t_settings_scr.settings_active = t_settings_scr.select_avatar;
    gtk_box_pack_start(GTK_BOX(menu_block), t_settings_scr.settings_active, FALSE, FALSE, 0);
